...
export default function Section({ title }) {
  ...
  useEffect(() => {
    async function fetchShows() {
      try {
        setLoading(true)
        const data = await getTopPopularMixed()
  ...
  if (loading) {
    return (
      <div
        data-layer="Section"
        className="self-stretch w-full px-12 pt-8 pb-8 flex-col mt-16 justify-start items-start gap-4 flex"
      >
    ...
    >
      <div data-layer="Container" className="w-full flex justify-between items-center">
        ...
        <div
          ...
          data-hover="false"
          data-variant="1"
          className="flex-col justify-start items-start inline-flex ml-4"
        >
