...
export async function POST(request) {
  try {
    ...
    try {
      // Llamada a Laravel API para obtener lo que el usuario ha completado
      const laravelResponse = await fetch(`${process.env.LARAVEL_API_URL}/api/user/media/completed`, {
        method: 'GET',
        headers: {
      ...
      if (!laravelResponse.ok) {
        ...
        return NextResponse.json(
          { error: 'Failed to fetch completed content from Laravel API' }, 
      ...
      if (!data || !data.success || !Array.isArray(data.data)) {
        console.log('No completed content found in Laravel response');
        return NextResponse.json([]);
      }
        ...
        const fetches = itemsArray.map(async (item) => {
          ...
          try {
            ...
            return {
              ...
              // Agregar información específica para contenido completado
              status: 'completed',
